...
if (isset($_POST['xAccion2'])) {
    if ($_POST["xAccion2"] == "upload") {
        ...
        $cve_tramite = isset($_POST['xCveTramite']) ? $_POST['xCveTramite'] : 0;

        $target_dir = "../documentos/tramites/";
        $target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
        $fileType = pathinfo($target_file, PATHINFO_EXTENSION);
        $size = $_FILES["fileToUpload"]["size"];

        /* RENOMBRADO DEL ARCHIVO CON LA CVE_PRODUCTO */
        $file_name_new = ("$cve_tramite.$fileType");
        ...
        if ($fileType != "pdf") {
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file_new)) {
                ...
                $sql = "UPDATE tramites SET pdf = '" . $valor . "' WHERE cve_tramite = $cve_tramite";
        ...
        if (!$exito) {
            ...
            header('Location:cat_tramites.php');
            return;
        }
    }
}
...
<div class="modal-header">
    ...
    <h4 class="modal-title">Trámite(s) | Subir PDF</h4>
</div>
    ...
    <div class="te">
        <form role="form" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" id="frmUpload" name="frmUpload">
            <div class="form-group">
                <input type="hidden" id="xCveTramite" name="xCveTramite" value="<?php echo($_GET["xCveTramite"]); ?>" />
